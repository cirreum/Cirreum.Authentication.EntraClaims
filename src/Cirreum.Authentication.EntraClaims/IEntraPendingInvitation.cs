...
/// <see cref="EntraUserProvisionerBase{TUser}.RedeemInvitationAsync"/>.
/// </summary>
/// <remarks>
/// <para>
/// Implement this interface on the invitation entity your application stores in its database.
/// It is not used as a generic constraint by <see cref="EntraUserProvisionerBase{TUser}"/> —
/// the find, expiry check, and claim are performed atomically inside
/// <see cref="EntraUserProvisionerBase{TUser}.RedeemInvitationAsync"/>, giving the
/// implementation full control over the transaction boundary.
/// </para>
/// <para>
/// Phase 1 (onTokenIssuanceStart) marks the invitation as <em>Claimed</em> and creates
/// the user record. Phase 2 (in-app onboarding) fully redeems the invitation, completes
/// the user profile, and assigns the Entra app role via Graph API.
/// </para>
/// </remarks>
/// <example>
/// <code>
/// public sealed class Invitation : IEntraPendingInvitation {
...
public interface IEntraPendingInvitation {
	...
	/// </summary>
	/// <remarks>
	/// Used to match against <see cref="EntraClaimsContext.Email"/> during token issuance.
